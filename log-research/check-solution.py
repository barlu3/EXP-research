...
sweep is never mistaken for a certified solution.
...
SOL_BIN_RE = re.compile(r"^\s*\d+\s+z(T[12]_\d+)_(\d+)\s+\*?\s+(\d+)")
COUPLE_RE = re.compile(r"^\s*(c\d+_(?:lo|hi)): (T1_\d+) \+ (T2_\d+) (>=|<=) (-?[\d.]+)\s*$")
LINK_RE = re.compile(r"^\s*link(T[12]_\d+): (T[12]_\d+) (.+) = 0\s*$")
TERM_RE = re.compile(r"([-+]) ([\d.]+) z(T[12]_\d+)_(\d+)")
...
def load_values(paths, cands):
    """Resolve each T1/T2 to its exact candidate via the binary set to 1."""
    vals = {}
    for path in paths:
...
def main():
    ...
    print(f"loaded {len(vals)} values ({n_t1} T1, {n_t2} T2)")
    ...
    for line in args.lp.read_text().splitlines():
        ...
        if not m:
            continue
        name, a, b, op, bound = m.group(1), m.group(2), m.group(3), m.group(4), Decimal(m.group(5))
    ...
    for v in violations[:10]:
        print(f"  VIOLATION {v[0]}: {v[1]} + {v[2]} {v[3]} {v[4]} -> {v[5]} (slack {v[6]:.3e})")
    if len(violations) > 10:
        print(f"  ... {len(violations) - 10} more")

    print(f"checked {checked} coupling rows ({skipped} skipped for missing vars)")
